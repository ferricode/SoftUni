    ...
    public static class Deserializer
    {
        public static string ImportGames(VaporStoreDbContext context, string jsonString)
        {
            ...
            foreach (var game in games)
            {
                ...
                if (!DateTime.TryParse(game.ReleaseDate, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime releaseData))
                {
                    sb.AppendLine("Invalid Data");
                    continue;
                }
        ...
        public static string ImportUsers(VaporStoreDbContext context, string jsonString)
        {
            ...
            foreach (var user in users)
            {
                ...
                {
                    string[] validType = new string[] { "Credit", "Debit" };

                    if (!IsValid(card) || validType.Any(t => t == card.Type) == false)
                    {
                        hasInvalidCard = true;
                        break;
                    }
