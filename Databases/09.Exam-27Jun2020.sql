...
SELECT * 
FROM Mechanics
WHERE [FirstName]='Ryan' AND LastName='Harnos' --AND Status IN ('Pending')
...
SELECT CONCAT(FirstName,' ',LastName) AS Available
	...
	LEFT JOIN Jobs AS j ON m1.MechanicId=j.MechanicId
...
SELECT *
	FROM (SELECT     p.PartId,
		  ...
		  LEFT JOIN Orders AS o ON pn.JobId = o.JobId
		  LEFT JOIN OrderParts AS op ON o.OrderId=op.OrderId
