namespace _01.Red_Black_Tree
{
        ...
        : IBinarySearchTree<T> where T : IComparable
    {
        ...
        public int Count { get => this.root != null ? this.root.Count : 0; }//his.NodeCount(this.root); }
        ...
        private int NodeCount(Node root)
        {
            ...
            // var nodeValue = this.IsRed(root) ? 0 : 1;
        ...
        private Node Insert(T element, Node node)
        {
            ...
            node.Count = this.NodeCount(node);
        ...
        {
            var node = this.Search(element, this.root);

            if (node != null)
            {
                var resultTree = new RedBlackTree<T>();
                resultTree.EachInOrder(x => resultTree.Insert(x), node);
                return resultTree;
            }
        ...
        private Node Delete(T element, Node node)
        {
            ...
            else
            {
                ...
                var copyNode = node;
                node = this.FindSmallestChhild(copyNode.Right);
        ...
        private Node FindSmallestChhild(Node node)
        {
            if (node.Left == null)
            {
                return node;
            }
            return FindSmallestChhild(node.Left);
        }
